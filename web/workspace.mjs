import {FileSystem,HOME,basename,parentPath,resolvePath,displayPath,tokenize} from './filesystem.mjs';
export function createWorkspace({openApp,toast}){
 ...
 function renderNotes(body){
  ...
  $('#export-note').onclick=()=>guard(()=>download());$('#new-note').onclick=()=>guard(()=>newNote(parentPath(fs.data.active)));
 }
 ...
 function complete(input){
  const line=input.value;let tokens;try{tokens=tokenize(line);}catch{return;}
  if(!tokens.length)return;
  const trailing=/\s$/.test(line),part=trailing?'':tokens.at(-1).value;
  const quote=s=>'"'+s.replace(/[\\"]/g,'\\$&')+'"';
  let candidates;
  if(tokens.length===1&&!trailing)candidates=['help','pwd','ls','cd','tree','mkdir','touch','cat','mv','cp','echo','open','edit','download','apps','history','date','whoami','clear'].filter(c=>c.startsWith(part));
 ...
 function execute(line){
  ...
  switch(cmd){
   ...
   case 'ls':count(0,1);{const p=args.length?path():cwd,n=fs.get(p);print(n.type==='file'?basename(p):fs.list(p).map(([p,n])=>basename(p)+(n.type==='directory'?'/':'')).join('\n')||'(empty)');}break;
