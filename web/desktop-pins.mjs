...
export function setupDesktopPins({apps,icon,toast}){
 ...
 function bounds(){return {w:Math.max(0,desktop.clientWidth-94),h:Math.max(0,desktop.clientHeight-185)};}
 ...
 function render(){const {w,h}=bounds();layer.innerHTML=pins.map(pin=>{const app=apps.find(a=>a.id===pin.id);return `<div class="desktop-pin" style="left:${8+pin.x*w}px;top:${8+pin.y*h}px"><button class="pin-launch" data-open="${app.id}" aria-label="Open ${app.name}" title="Drag to reposition">${icon(app)}<span>${app.name}</span></button><button class="unpin" data-unpin="${app.id}" aria-label="Unpin ${app.name} from desktop" title="Unpin from desktop">×</button></div>`;}).join('');}
