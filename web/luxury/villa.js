...
function exitWalk(){if(!walking)return;walking=false;keys.clear();walkVelocity.set(0,0,0);jumpHeight=jumpVelocity=0;grounded=true;camera.position.y=1.67;camera.rotation.z=0;if(controls)controls.enabled=true;$('#walk').textContent='Walk inside';$('.hint').textContent='Drag to explore · Scroll or pinch to zoom';status('First-person walk ended.');}
function startWalk(){if(!model||!renderer)return;stop();walking=true;walkPhase=jumpHeight=jumpVelocity=0;grounded=true;walkVelocity.set(0,0,0);controls.enabled=false;camera.position.set(8.7,1.67,-15);camera.rotation.order='YXZ';yaw=0;pitch=0;camera.rotation.set(0,0,0);$('#walk').textContent='Exit walk';$('.hint').textContent='WASD to walk · Shift to move faster · Space to jump · Mouse to look · Esc to exit';$('#scene-caption').textContent='First-person walkthrough from the entry.';renderer.domElement.requestPointerLock?.();status('Walk mode: WASD, Shift, Space, and mouse.');}
...
document.addEventListener('keydown',event=>{keys.add(event.code);if(walking&&event.code==='Space'){event.preventDefault();if(grounded){grounded=false;jumpVelocity=4.65;}}});document.addEventListener('keyup',event=>keys.delete(event.code));
