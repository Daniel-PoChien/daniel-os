...
let walking=false,yaw=0,pitch=0,walkPhase=0;const keys=new Set(),walkRay=new THREE.Raycaster(),walkDirection=new THREE.Vector3(),walkRight=new THREE.Vector3(),walkVelocity=new THREE.Vector3();
...
function exitWalk(){if(!walking)return;walking=false;keys.clear();walkVelocity.set(0,0,0);camera.position.y=1.67;camera.rotation.z=0;if(controls)controls.enabled=true;$('#walk').textContent='Walk inside';$('.hint').textContent='Drag to explore · Scroll or pinch to zoom';status('First-person walk ended.');}
function startWalk(){if(!model||!renderer)return;stop();walking=true;walkPhase=0;walkVelocity.set(0,0,0);controls.enabled=false;camera.position.set(8.7,1.67,-15);camera.rotation.order='YXZ';yaw=0;pitch=0;camera.rotation.set(0,0,0);$('#walk').textContent='Exit walk';$('.hint').textContent='WASD to walk · Shift to move faster · Mouse to look · Esc to exit';$('#scene-caption').textContent='First-person walkthrough from the entry.';renderer.domElement.requestPointerLock?.();status('Walk mode: use WASD, Shift, and the mouse.');}
function updateWalk(delta){if(!walking)return;const forward=(keys.has('KeyW')?1:0)-(keys.has('KeyS')?1:0),side=(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0),moving=forward||side;walkDirection.set(0,0,0);if(moving){walkDirection.set(-Math.sin(yaw),0,-Math.cos(yaw)).multiplyScalar(forward);walkRight.set(Math.cos(yaw),0,-Math.sin(yaw)).multiplyScalar(side);walkDirection.add(walkRight).normalize().multiplyScalar(keys.has('ShiftLeft')||keys.has('ShiftRight')?4.8:3.2);}walkVelocity.lerp(walkDirection,1-Math.exp(-10*delta));const speed=walkVelocity.length();if(speed>.04){const heading=walkVelocity.clone().normalize();walkRay.set(new THREE.Vector3(camera.position.x,1.67,camera.position.z),heading);walkRay.far=.48+speed*delta;const blocked=walkRay.intersectObject(model,true).some(hit=>!/(leaf|canopy|water)/i.test(hit.object.material?.name||''));if(!blocked)camera.position.addScaledVector(walkVelocity,delta);else walkVelocity.set(0,0,0);walkPhase+=speed*delta*5.2;}const blend=1-Math.exp(-12*delta),bob=speed>.12?Math.sin(walkPhase)*Math.min(.045,speed*.012):0,sway=speed>.12?Math.sin(walkPhase*.5)*.008:0;camera.position.y=THREE.MathUtils.lerp(camera.position.y,1.67+bob,blend);camera.rotation.set(pitch,yaw,sway);}
...
document.addEventListener('pointerlockchange',()=>{if(walking&&document.pointerLockElement!==renderer?.domElement)exitWalk();});
...
document.addEventListener('keydown',event=>keys.add(event.code));document.addEventListener('keyup',event=>keys.delete(event.code));
