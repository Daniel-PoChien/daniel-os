...
const mobileStyle=document.createElement('style');mobileStyle.textContent='.views[hidden]{display:none!important}#viewport[data-walking=true] .hint{display:none}#mobile-controls{position:absolute;z-index:4;left:18px;right:18px;bottom:max(22px,env(safe-area-inset-bottom));display:flex;align-items:flex-end;justify-content:space-between;pointer-events:none}.move-pad{display:grid;grid-template-columns:repeat(3,54px);grid-template-rows:repeat(2,54px);gap:7px}.move-pad button,#mobile-jump{pointer-events:auto;border:1px solid #e5c994;background:#142832e8;color:#f5e7cf;box-shadow:0 3px 12px #0006;touch-action:none;user-select:none;-webkit-user-select:none}.move-pad button{position:relative;width:54px;height:54px;border-radius:50%;padding:0}.move-pad button:before{content:"";position:absolute;left:50%;top:50%;width:0;height:0;transform:translate(-50%,-50%);border-style:solid}.move-pad button[data-move=KeyW]{grid-column:2}.move-pad button[data-move=KeyW]:before{border-width:0 8px 12px;border-color:transparent transparent currentColor}.move-pad button[data-move=KeyA]{grid-column:1;grid-row:2}.move-pad button[data-move=KeyA]:before{border-width:8px 12px 8px 0;border-color:transparent currentColor transparent transparent}.move-pad button[data-move=KeyS]{grid-column:2;grid-row:2}.move-pad button[data-move=KeyS]:before{border-width:12px 8px 0;border-color:currentColor transparent transparent}.move-pad button[data-move=KeyD]{grid-column:3;grid-row:2}.move-pad button[data-move=KeyD]:before{border-width:8px 0 8px 12px;border-color:transparent transparent transparent currentColor}#mobile-jump{width:76px;height:76px;border-radius:50%;font-size:13px;font-weight:700;letter-spacing:.4px}#mobile-controls button:active{background:#d9bd92;color:#17252d;transform:scale(.94)}#mobile-controls[hidden]{display:none!important}';document.head.append(mobileStyle);
...
function exitWalk(){if(!walking)return;walking=false;host.dataset.walking='false';keys.clear();walkVelocity.set(0,0,0);jumpHeight=jumpVelocity=0;grounded=true;camera.position.y=1.67;camera.rotation.z=0;if(controls)controls.enabled=true;$('#walk').textContent='Walk inside';$('.views').hidden=false;$('#mobile-controls').hidden=true;$('.hint').textContent='Drag to explore · Scroll or pinch to zoom';status('First-person walk ended.');}
function startWalk(){if(!model||!renderer)return;stop();walking=true;host.dataset.walking='true';walkPhase=jumpHeight=jumpVelocity=0;grounded=true;walkVelocity.set(0,0,0);controls.enabled=false;camera.position.set(8.7,1.67,-15);camera.rotation.order='YXZ';yaw=0;pitch=0;camera.rotation.set(0,0,0);$('#walk').textContent='Exit walk';$('.views').hidden=true;$('#mobile-controls').hidden=!touchMode;$('.hint').textContent=touchMode?'Use arrows to walk · Drag the scene to look · Tap Jump':'WASD to walk · Shift to move faster · Space to jump · Mouse to look · Esc to exit';$('#scene-caption').textContent='First-person walkthrough from the entry.';if(!touchMode)renderer.domElement.requestPointerLock?.();status(touchMode?'Touch walk mode enabled.':'Walk mode: WASD, Shift, Space, and mouse.');}
