...
let walking=false,yaw=0,pitch=0;const keys=new Set(),walkRay=new THREE.Raycaster(),walkDirection=new THREE.Vector3(),walkRight=new THREE.Vector3();
...
function exitWalk(){if(!walking)return;walking=false;keys.clear();if(controls)controls.enabled=true;$('#walk').textContent='Walk inside';$('.hint').textContent='Drag to explore · Scroll or pinch to zoom';status('First-person walk ended.');}
function startWalk(){if(!model||!renderer)return;stop();walking=true;controls.enabled=false;camera.position.set(8.7,1.67,-15);camera.rotation.order='YXZ';yaw=0;pitch=0;camera.rotation.set(0,0,0);$('#walk').textContent='Exit walk';$('.hint').textContent='WASD to walk · Mouse to look · Esc to exit';$('#scene-caption').textContent='First-person walkthrough from the entry.';renderer.domElement.requestPointerLock?.();status('Walk mode: use WASD and the mouse.');}
function updateWalk(delta){if(!walking)return;const forward=(keys.has('KeyW')?1:0)-(keys.has('KeyS')?1:0),side=(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0);if(!forward&&!side)return;walkDirection.set(-Math.sin(yaw),0,-Math.cos(yaw)).multiplyScalar(forward);walkRight.set(Math.cos(yaw),0,-Math.sin(yaw)).multiplyScalar(side);walkDirection.add(walkRight).normalize();const distance=2.1*delta;walkRay.set(camera.position,walkDirection);walkRay.far=.48;const blocked=walkRay.intersectObject(model,true).some(hit=>!/(leaf|canopy|water)/i.test(hit.object.material?.name||''));if(!blocked)camera.position.addScaledVector(walkDirection,distance);camera.position.y=1.67;}
